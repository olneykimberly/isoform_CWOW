...
project_ID <- "flair"
c(metadata$NPID, metadata$group, metadata$RIN)
```
...
# Save object
```{r}
saveRDS(dge, paste0("../../rObjects/", project_ID, "_raw_DEGobject.rds"))
...
# Run the DESeq2 workflow to get dispersion estimates
# Note: Since you only have 3 vs 3, DESeq2 will rely heavily on shrinkage (good for estimating parameters)
dds <- DESeq(dds)

# Extract the fitted parameters for PROPER
deseq_params <- list(
  means = rowMeans(counts(dds, normalized=TRUE)), # Mean of normalized counts
  dispersions = mcols(dds)$dispFit # Fitted dispersion values (shrunken)
)
summary(deseq_params$dispersions)
saveRDS(dds, paste0("../../rObjects/", project_ID, "_raw_DESeqObject.rds"))
